...
// 加载table部分
loadTablecontentReportdynamic({});

function loadTablecontentReportdynamic(data) {
    for (var i in data) {
        if (data[i] !== '') {
            dataSource[i] = data[i];
        }
    ...
    $.ajaxFunc(source, fillTableSReportdynamic, errorHandle);
}

function fillTableSReportdynamic(data, status, xhr) {
    var strGet = getTableReportDynamic(data);
...
function getTableReportDynamic(data) {
    var totalCount = getTotal(data);
    var str_main = "";
    str_main += "<tr><th>" + "陪我号" + "</th><th>" + "昵称" + "</th><th>" + "图片" + "</th><th>" + "文案" + "</th><th>" + "更新时间" + "</th><th>" + "选中" + "</th>";
...
$('.mode').on('click', function () {
    ...
    loadTablecontentReportdynamic(data_send);
});
...
function pageSearch() {
    ...
    loadTablecontentReportdynamic(data);
}
            ...
            if (option === 1) {
                ...
                if (option_tuids.length > 0) {
                    for (var i = 0; i < option_tuids.length; i++) {
                        ...
                        $.ajaxFunc(sourceBan, successDeal, errorHandle);
                    }
                }
            }
        }
...
function send_message(type) {
    ...
    $.ajaxFunc(sourceTo, successWarn, errorHandle);
}
function successWarn(data, status, xhr) {
    if (data.code === 0) {
        if (option_tuids.length > 0) {
            for (var i = 0; i < option_tuids.length; i++) {
                var data_message = {
