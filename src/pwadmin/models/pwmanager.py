...
class PwManager(models.Model):
    ...
    @property
    def sdk(self):
        return cache.get('user_sdk_{}'.format(self.uid))

    def set_token(self, token):
        cache.set('user_token_{}'.format(self.uid), token, settings.SESSION_COOKIE_AGE)

    def set_sdk(self, token):
        cache.set('user_sdk_{}'.format(self.uid), SneakSDK(host=settings.API_HOST, user=self, token=token))
